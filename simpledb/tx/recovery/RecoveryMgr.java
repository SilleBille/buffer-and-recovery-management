...
public class RecoveryMgr {
	...
	private void doRecover() {
		...
		while (iter.hasNext()) {
			...
			} else if (!commitedTxs.contains(rec.txNumber()) || !rollbackTxs.contains(rec.txNumber())) {
				rec.undo(txnum);
				//System.out.println("undo: " + rec);
			}
		}

		Iterator<LogRecord> forwardIter = new LogRecordForwardIterator((LogRecordIterator)iter);
		while (forwardIter.hasNext()) {
			LogRecord rec = forwardIter.next();
			if (commitedTxs.contains(rec.txNumber())) {
				rec.redo(txnum);
				//System.out.println("redo: " + rec.toString());
			}
		}
	}
